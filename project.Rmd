...
booking_train %>%
  ...
  select(-arrival_date_day_of_month, -babies, -booking_changes, -days_in_waiting_list, -required_car_parking_spaces) %>%
  cor() %>%
  corrplot(order= "AOE", col = COL2("PiYG"), diag = FALSE)

```

```{r}
set.seed(1234)
corr_here <- cor(hotel_booking)
```
...
days_in_waiting_list, arrival_date_year, arrival_date_year, assigned_room_type, booking changes, reservation_status, country, days_in_waiting_list,
